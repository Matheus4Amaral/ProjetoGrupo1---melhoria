...
export default function MeusVeiculos() {
  ...
  function handleRemover(veiculo) {
    setVeiculoParaRemover(veiculo);
  }

  async function confirmarRemocao() {
    const veiculo = veiculoParaRemover;
  ...
  return (
    <section className="veiculos">
      ...
      <form className="veiculos-form" onSubmit={handleSalvar}>
        ...
        {erroForm && (
          <div className="veiculos-aviso veiculos-aviso--erro">{erroForm}</div>
        )}
        {sucesso && (
          <div className="veiculos-aviso veiculos-aviso--sucesso">
            {sucesso}
          </div>
        )}
      ...
      <div className="veiculos-lista-cabecalho">
        <h2>Cadastrados</h2>
        {temVeiculos && (
          <span className="veiculos-contagem">{veiculos.length}</span>
        )}
      </div>

      {erro && (
        <div className="veiculos-aviso veiculos-aviso--erro">{erro}</div>
      )}

      {carregando && (
        <div className="veiculos-estado">Carregando veículos...</div>
      )}

      {!carregando && !erro && !temVeiculos && (
        <div className="veiculos-estado">
            ...
            >
              ...
              <div className="veiculos-cartao-info">
                ...
                <span className="veiculos-cartao-detalhes">
                  {[veiculo.ano, veiculo.cor, veiculo.combustivel]
                    .filter(Boolean)
                    .join(" · ") || "Sem detalhes"}
                </span>
              </div>
              ...
              <div className="veiculos-cartao-acoes">
                ...
                >
                  <svg width="16" height="16" viewBox="0 0 24 24" fill="none">
                    <path
                      d="M4 20h4L18.5 9.5a2.12 2.12 0 0 0-3-3L5 17v3M13.5 6.5l3 3"
                      stroke="currentColor"
                      strokeWidth="1.8"
                      strokeLinecap="round"
                      strokeLinejoin="round"
      ...
      <ConfirmModal
        ...
        mensagem={
          veiculoParaRemover
            ? `Remover o veículo de placa ${veiculoParaRemover.placa}?`
            : ""
        }
        textoConfirmar="Remover"
        textoCancelar="Cancelar"
        onConfirmar={confirmarRemocao}
        onCancelar={() => setVeiculoParaRemover(null)}
      />
    </section>
  );
}
